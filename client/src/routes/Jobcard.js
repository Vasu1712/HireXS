...
import { makeUnauthenticatedDELETERequest } from '../utils/serverHelper';
...
export function Jobcard({
	...
	const handleDelete = async () => {
		const url = "/jobslist/jobs/delete/" + JobId;
		const data = { JobId: JobId }
		const response = await makeUnauthenticatedDELETERequest(
			url,
			data
		);
		...
		<Card className='mt-8 w-1/4 bg-black'>
			<CardBody>
				...
				<div className='text-white mt-4 flex'>
					...
					/>
					<div className='pl-2'>
						Expires: {format(new Date(LastDate), 'dd MMM yyyy')}
					</div>
				</div>
			</CardBody>
